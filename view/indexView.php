...
<body onload="captchaCF2M(submitForm, 7);">
    ...
    ?>
        ...
        <form id='monFormulaire' name='lemail' action='' method="POST">
            ...
            <a href="#">(*) Ce champ est obligatoire</a>
        </form>
        </div>
        <br>
        <div class="capt">
    ...
    # on a au moins un mail
        ...
        ?>
    <h4>Nous avons <?=$nbMail?> messages envoyés</h4>
        <?php
        # tant qu'on a des mail
        foreach($responseMail as $item):
        ?>
<div class='theMail'><?=$item['firstname']?> <?=$item['lastname']?> <?=$item['usermail']?> <?=$item['message']?></div>
        <?php
        endforeach;
    endif;
    ?>
